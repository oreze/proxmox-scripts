...
2. Create a new role with needed privileges:
    ...
    ```sh
    pveum role add TerraformDeploy -privs "VM.Allocate VM.Clone VM.Config.CDROM VM.Config.CPU VM.Config.Cloudinit VM.Config.Disk VM.Config.HWType VM.Config.Memory VM.Config.Network VM.Config.Options VM.Audit VM.PowerMgmt Sys.Audit Datastore.AllocateSpace Datastore.Audit"
    ```
